...
var offsetTargetY = 0;
if place_meeting(targetX,targetY+1,obj_collision)
{offsetTargetY = 1;}
...
for (var j = 0; j < room_height; ++j) {
	...
	if (collision_line(x+(xspd*20),ground_y1,x+(xspd*30),ground_y1,obj_collision,0,0))
	|| (collision_line(x+(xspd*20),ground_y1,x+(xspd*30),ground_y1,obj_void,0,0))
	{
		break;
	}
}
...
if (ground_y-jump_rangeY > obj_flowerl.bbox_bottom)
{
	if (colly)
	{
...
if mp_grid_path(obj_grid.grid,path,x,y-offsetY,targetX,targetY-offsetTargetY,true)
{
	...
	if place_meeting(x,y+1+yspd,obj_collision)
	{
		var crlY = 9
		if (ground_y > obj_flowerl.bbox_bottom-2)
		{
			crlY = 0;
		}
		...
		if (y > yy+crlY)
		{
			...
			|| (!collision_rectangle(bbox_left+(xspd*pit_rangeX),ground_y1,bbox_right+(xspd*pit_rangeX),ground_y1+1,obj_collision,false,true) && (ground_y1 >= obj_flowerl.bbox_bottom-1))
			{yspd = -jspd;}
		}
		else
		{
			...
			if (!collision_rectangle(bbox_left+(xspd*pit_rangeX),ground_y1,bbox_right+(xspd*pit_rangeX),ground_y1+1,obj_collision,false,true) && (ground_y1 >= obj_flowerl.bbox_bottom-1))
			...
			&& (inst != noone) && (ground_y >= obj_flowerl.bbox_bottom))
			//
...
if place_meeting(x+(xspd*spd),y-1-should_jump,obj_collision)
{
	xspd = 0;
	x = round(x/2)*2;
}
...
{
	yspd = 0;
	if collision_rectangle(bbox_left,bbox_bottom,bbox_right,bbox_bottom+1+yspd,obj_collision,false,true)
	{should_jump = 1}
	else{y = round(y/2)*2;x = round(x/2)*2;}
}
else if !instance_place(x,y+1+yspd,obj_collision)
{
	if (yspd < term_vel)
	{yspd += grav}else
	{yspd = term_vel}
